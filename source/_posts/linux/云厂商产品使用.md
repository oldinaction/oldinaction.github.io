...
- [vercel](https://vercel.com/) 免费部署前端网站
    - Hobby 免费套餐: 100GB 流量
    - 免费版无法导入 github 私有仓库
- Vercel CLI 手动部署
...
# 在项目目录执行: 按照提示配置 - 自动上传当前目录 - 自动构建 - 自动部署
vercel
# 改代码后重新部署即可; --prod 表示部署到生产环境, 不加则部署到预览环境
vercel --prod
...
GitHub Actions
↓
Vercel Deploy Hook / CLI

# 添加环境变量(后端需要用到的私有密钥等)，或 Dashboard 的 Settings → Environment Variables 里添加
vercel env add SUPABASE_SERVICE_ROLE_KEY
# 然后粘贴值，选择 Production / Preview / Development 环境
```
...
- https://www.netlify.com/ 免费部署前端网站
