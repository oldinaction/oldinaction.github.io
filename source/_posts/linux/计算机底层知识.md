...
cat fd-file xxx 1> cat.out 2>& 1 # 将标准输出和错误输出指向同一文件。重定向符前面是类型，后面一般是文件，如果不是文件则需要加重定向符后面加&；且2>& 1必须在后面，因为需要先把1指向cat.out，1才进行了定义
read a # 默认标准输入为键盘
...
## 管道符和父子进程
# 显示文件第2行
head -2 fd-file | tail -1 # def. 管道为把左边的输出给右边作为输入
# 父子进程
x=100
/bin/bash # 创建并进入子进程
...
- [C10K问题，即单机1万个并发连接问题](http://www.kegel.com/c10k.html) (一#184)
    ...
    - NIO解决的问题和缺点
        ...
        - 仍存在的缺点：每循环一次，read为O(n)复杂度，很多read调用是无意义的(客户端并没有发数据过来)，但是每次回产生系统调用
    - 多路复用器(如netty中的)
