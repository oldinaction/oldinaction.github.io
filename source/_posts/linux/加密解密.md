...
```bash
## (推荐使用certbot-auto，见下文)安装，可根据不同环境获取不同安装命令。参考：https://certbot.eff.org/lets-encrypt/centosrhel7-nginx
sudo yum install certbot python2-certbot-nginx # python2-certbot-nginx为基于nginx自动安装证书的插件
certbot -h # 查看帮助
certbot --help all # 查看所有帮助

## 安装certbot-auto(是对certbot的封装，执行时可动态下载依赖并添加到python虚拟环境中)
wget https://dl.eff.org/certbot-auto # 下载到本地
chmod a+x ./certbot-auto # 添加可执行权限
./certbot-auto -h # 查看帮助

## 参数说明
run # 获取并安装证书到当前的Web服务器，默认选项，`./certbot-auto run` 和 `./certbot-auto` 效果一致
...
revoke          # 撤销证书，certbot-auto revoke --cert-path /path/to/fullchain.pem [options]
delete          # 删除证书
...
## 手动获取证书
./certbot-auto certonly --manual -d test.aezo.cn
...
./certbot-auto --nginx -d test.aezo.cn

## 自动更新脚本
crontab -e
# 在每天凌晨3点运行，该命令将检查服务器上的证书是否将在未来30天内过期，如果是，则进行更新
0 3 * * * /root/letsencrypt/certbot-auto renew --quiet # --quiet 指令告诉 certbot 不要生成输出
...
- 执行exe文件 - Create new cerificate (full options) - Manual input - 输入host name - 选择默认认证方式 - serve verification files from memory - rsa key - pem encoded files(Apache, nginx, etc.) - 输入文件存放路径 - No store steps
- 将生成的文件设置到ngixn(参考上文手动配置nginx证书)
- 再次执行exe文件 -  A: Manage renewals (1 total) 可查看托管的自动更新证书
