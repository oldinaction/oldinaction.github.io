...
```bash
## 安装(certbot-auto已经停止服务)。参考：https://certbot.eff.org/lets-encrypt/centosrhel7-nginx
...
# 如果是阿里云的域名，则修改 ALY_KEY/ALY_TOKEN 的Token(可使用主账号的AccessKey；或者创建一个子账号，然后增加AliyunDNSFullAccess云解析权限，然后使用子账号的AccessKey)
vi au.sh
...
## 部署证书
# (七牛上传)证书路径在 /etc/letsencrypt/live，复制 fullchain.pem 和 privkey.pem 到普通用户目录，并设置文件权限，然后下载上传到如七牛即可
