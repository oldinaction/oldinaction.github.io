...
- 技术名词
  ...
  - **回表**：如通过普通索引找到的数据(B+树的 data 值)为主键值，需要获取其他数据时则需要根据主键索引重新检索。`select id, name, sex from user where name = 'smalle'` 此时需要回表查询sex（name为索引）
