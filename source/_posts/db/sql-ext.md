...
-- sqlserver
-- 23: YYYY-MM-DD,   20(120): YYYY-MM-DD HH:MI:SS,   108: HH:MI:SS,   111:YYYY/MM/DD,   112:YYYYMMDD
select convert(varchar(19), getdate(), 20); -- 格式化日期(20为一种格式) 2000-01-01 12:00:00
select convert(varchar(10), getdate(), 120); -- 格式化日期 2000-01-01
...
select * from users where last_name is null and name is not null;
select nvl(content, 'hello') from dual; -- null或空字符串则设置默认值

-- mysql: null不包含空字符串('' != null)
select * from users where (last_name is null or last_name = '') and (name is not null and name != '');
-- 终极判断: (case when #{name} is null then #{default} else if(#{name}='', #{default}, #{name}) end)
select ifnull(null, 1), ifnull('', 1), if(''='', 1, 0); -- 1 '' 1
select coalesce(email, email_bak, '无邮箱'); -- 仅判断 NULL, 指定多个默认值
-- 类似 nvl 效果
select isnull(nullif(content, ''), 'hello');  -- NULLIF: 字段值与比较值(此处的空字符串)相等，则返回 NULL，否则返回字段本身

-- sqlserver: null不包含空字符串('' != null), 判断同mysql
select * from users where (last_name is null or last_name = '');
select isnull(content, 'hello'); -- null则设置默认值(不含空字符串);
select COALESCE(email, email_bak, '无邮箱'); -- 仅判断 NULL, 指定多个默认值
-- 类似 nvl 效果
select ISNULL(NULLIF(LTRIM(RTRIM(content)), ''), 'hello');
```
- 空值排序
...
```sql
concat(t.id, '-', t.name, '-', t.age) -- mysql 
t.id || '-' || t.name || '-' || t.age -- oracle
-- sqlserver
concat('姓名：', NULL, '李四', '，年龄：', 30) -- 输出：姓名：李四，年龄：30
t.id + '-' + t.name + '-' + t.age -- 需注意 NULL 处理(有一个为 NULL 则全为 NULL)和数据类型转换
concat_ws('-', '中国', NULL, '上海市', '浦东新区') -- 中国-上海市-浦东新区
```
- 字符串类型值
...
- 合并到一个字段
    - 参考Oracle [wm_concat行转列](#wm_concat行转列)
      - SqlServer的 `STRING_AGG`, 用法一致; 低版本可使用 FOR XML PATH (参考下文)
...
SELECT 
    ...
    STUFF(
        (
            ...
            WHERE ud.user_id = t.id  -- 关联外层的用户ID
            FOR XML PATH(''), TYPE  -- 生成无标签XML，TYPE避免转义
        ).value('.', 'NVARCHAR(MAX)'),  -- 转换为普通字符串
        1, 1, ''  -- 移除第一个字符（多余的逗号）
    ) AS depart_names  -- 合并后的部门名称
FROM sys_user t 
WHERE t.type = 0;
```
...
WITH PermissionTree AS (
    -- 锚点成员：查询根节点（parent_id 为空字符串的第一层数据）
    SELECT 
        id, 
        parent_id, 
        name, 
        1 AS level,  -- 层级标识，根节点为1
        CAST(id AS VARCHAR(MAX)) AS tree_path  -- 树形路径（可选，用于排序）
    FROM sys_permission 
    WHERE del_flag = 0 AND parent_id = ''  -- 根节点条件：parent_id 为空字符串
    ...
    SELECT 
        ...
        t.parent_id, 
        t.name, 
        pt.level + 1 AS level,  -- 子节点层级=父节点层级+1
        pt.tree_path + '/' + CAST(t.id AS VARCHAR(MAX)) AS tree_path  -- 拼接路径
    FROM sys_permission t
    INNER JOIN PermissionTree pt ON t.parent_id = pt.id  -- 子节点parent_id关联父节点id
    WHERE t.del_flag = 0
)
-- 最终查询结果，按层级和路径排序
SELECT 
    ...
    level,
    tree_path,
    REPLICATE('    ', level - 1) + name AS name_with_indent  -- 缩进显示（更直观）
FROM PermissionTree
ORDER BY tree_path;
```
...
- **`between...and` 左右边界都包含。**当处理时间时(类型为datetime)，语句`between '2018-10-01' and '2018-11-01'`，实际执行`between 2018-10-01 00:00:00 and 2018-11-01 00:00:00`，从而少算了11-1的数据。解决办法
    - 写全时分秒，2018-10-01 00:00:00.000 至2018-10-01 23:59:59.999
...
select length('你'), lengthb('你'), lengthb('你123Abc')
    ,substr('你123', 1, 3), substrb('你123', 1, 3), rtrim('abc,', ',') from dual; -- 1 3 9 你12 你1 abc
...
-- substr 截取子符串; 对应基于字符的则是 substrb
...
select
    ...
    -- (推荐)解决 to_char 超过 4000个字符报错问题； 貌似还是会报错，超过 4000 时把截取长度改成 4000 即可
...
- 字段类型设置成`NUMBER(10)`则可以存储 11 位二进制数来表示状态(是/否), 如默认值设置成 0 表示所有状态位全部关闭
    - 第 0 位: 1（2⁰）如存储状态 A
    - 第 1 位: 2（2¹）如存储状态 B
    - 第 2 位: 4（2²）如存储状态 C
    - 第 3 位: 8（2³）如存储状态 D
    ...
    - 第 10 位: 1024（2¹⁰）
- 核心位运算函数
    - 按位与 `BITAND(status_bits, 1)`：判断第 0 位是否为 1（状态 A 是否开启）
    - 按位或 `status_bits + 2 - BITAND(status_bits, 2)`：开启第 1 位（状态 B），开启指定位（无重复）	
    - 清除指定位 `status_bits - BITAND(status_bits, 8)`：关闭第 3 位（状态 D，设置为 0）
    - 按位异或 `BITXOR(status_bits, 4)`：切换第 2 位（状态 C），0->1, 1->0
- 使用

```sql
CREATE TABLE t_bit_demo (
    id NUMBER PRIMARY KEY,       -- 主键
    status_bits NUMBER(10) DEFAULT 0, -- 存储位状态的列，默认0（所有状态关闭）
    user_name VARCHAR2(50)      -- 示例业务列
);

-- 插入：状态A（1）+ 状态C（4）= 5（二进制101）
INSERT INTO t_bit_demo (id, status_bits, user_name) VALUES (1, 1+4, '张三');
-- 插入：状态B（2）+ 状态D（8）= 10（二进制1010）
INSERT INTO t_bit_demo (id, status_bits, user_name) VALUES (2, 2+8, '李四');
-- 插入：所有状态关闭（0）
INSERT INTO t_bit_demo (id, status_bits, user_name) VALUES (3, 0, '王五');

-- 查询状态 A（第 0 位，值 1）开启的所有用户
SELECT * FROM t_bit_demo WHERE BITAND(status_bits, 1) = 1; -- 2^0 = 0 表示关闭
-- 查询状态 B（第 2 位，值 4）或状态 D（第 3 位，值 8）开启的用户
SELECT * FROM t_bit_demo WHERE BITAND(status_bits, 4) = 4 OR BITAND(status_bits, 8) = 8;
-- 查询状态 A（1）和状态 C（4）同时开启的用户
SELECT * FROM t_bit_demo WHERE BITAND(status_bits, 1+4) = 1+4;

-- 更新, 避免重复加
-- 给王五（id=3）开启状态 B（第 1 位，值 2）
UPDATE t_bit_demo SET status_bits = status_bits + 2 - BITAND(status_bits, 2) WHERE id = 3;
-- 给李四（id=2）关闭状态 D（第 3 位，值 8）
UPDATE t_bit_demo SET status_bits = status_bits - BITAND(status_bits, 8) WHERE id = 2;
```
...
```sql
-- 去除空格
SELECT LTRIM(RTRIM('  张三  ')) AS 结果; -- 输出：'张三'
SELECT TRIM('  张三  ') AS 结果; -- TRIM: SQL Server 2017+ 支持
