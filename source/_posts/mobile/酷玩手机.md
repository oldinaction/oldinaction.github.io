...
- IOS文件管理
    - 文件 - 浏览 - 下载(如 Safire 下载的文件会显示在这里)
        - zip/rar等压缩文件, 分享到 Filza(基于巨魔安装) 进行解压
    - Filza
        - `/var/mobile/Containers/Shared/AppGroup/com.apple.mobile_container_m...ider.LocalStorage/File Provider Storage` 上文手机展示的文件目录(子目录 Downloads 为下载目录)
        - `/var/mobile/Containers/Shared/AppGroup/com.tigisoftware.sharedfiles/Doucuments` 为上文分享过来的文件夹
        - `/var/mobile/Containers/Data/Application/[AppID]/Documents` 应用生成/下载的文件
- **IOS上安装TrollStore(巨魔商店, 无需Root权限)**
    - 安装
...
- 巨魔App
    ...
    - 巨魔 NFC
- 安装 IPA 程序
    - 法一: MAC上安装`Apple Configurator 2 2.14.app`(无法安装 Flutter 打包的未签名 Runner.app)
        - 直接手机USB连接mac，即可通过mac来安装ipa格式手机app。但是像`Shadowrocket`新版本已经加入了账号验证，安装上去了必须验证cloud账号才能打开
        - 可将ipa文件上传到手机文件目录中：添加 - 文稿 - 选择文件夹上传文件。然后通过TrollStore安装
    - 法二: **通过TrollStore安装**(需手机提前安装TrollStore)
        - 将文件做成 HTTP 下载链接，通过浏览器下载到手机，然后在文件 - 我的iPhone - 下载中找到文件 - 长按 - 共享 - TrollStore
        - 或者通过文件App找到对应ipa，然后打开，再点击通过其他应用打开，选择TrollStore
        - 或者打开TrollStore，点击右上角加号 - install from URL
- IPA 打包: 参考[flutter.md#打包安装](/_posts/mobile/flutter.md#打包安装)
- IPA 下载地址
    - https://ipa.store/
    - https://decrypt.day/
