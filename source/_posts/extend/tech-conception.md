...
- 文件编码发展
    - `ASCII`: 美国标准信息交换码，只支持255个字节
        - `ANSI`编码: ANSI编码是一种对ASCII码的拓展，表示英文字符时用一个字节，表示中文用两个或四个字节。不同的国家和地区制定了不同的标准，由此产生了 GB2312、GBK、Big5、Shift_JIS等；在简体中文 Windows 操作系统中，ANSI 编码代表 GBK 编码；在繁体中文 Windows 操作系统中，ANSI 编码代表 Big5
    - `GB2312`: ASCII处理中文明显是不够的，中文不止255个汉字，所以中国制定了GB2312编码，用两个字节表示一个汉字。同理，日文，韩文也有自己的编码，如果出现多种语言混合显示就一定会出现乱码
        - `GBK`编码: GBK编码专门用来解决中文编码的，不论中英文都是双字节的。包含GB2312、GB18030，统统称他们叫做"DBCS"（Double Byte Charecter Set 双字节字符集）
        - GB2312: 当字符小于127位时，与ASCII的字符相同，但当两个大于127的字符连接在一起时，就代表一个汉字，第一个字节称为高字节（从0xA1-0xF7）,第二个字节为低字节（从0xA1-0xFE），这样大约可以组合7000多个简体汉字
        - GB18030: 基于GB2312，又加了近几千个少数民族的字符
    - `Unicode`: 于是Unicode出现了，将所有语言统一到一套编码中
        - `\u`开头的就是Unicode编码
    - `UTF-8`: 乱码问题解决了，但是如果内容全是英文，Unicode编码比ASCII需要多一倍的存储空间，同时如果传输需要多一倍的传输，所以出现了可变长的编码UTF-8，把英文变长一个字节，汉字3个字节，特别生僻的变成4-6字节，如果传输大量的英文，utf8作用就很明显
        - 一种 Unicode 的实现方式，其他实现方式还包括 UTF-16（字符用两个字节或四个字节表示）和 UTF-32（字符用四个字节表示）
    - 关于`BOM`: Byte Order Mark，就是字节序标记
        - UTF-8(带BOM)文件16进制打卡开头会有一个`EF BB BF`的字符就是BOM头
        - 尽管BOM 对于 UTF-8 编码的文件来说是可选的，并且不会改变 UTF-8 文本的解析方式，但在某些情况下，BOM 的存在可能会导致问题或不一致的行为
...
- PO、BO、VO、DTO、POJO。具体参考https://www.zhihu.com/question/39651928
    ...
    - `DTO` 传输对象，接口间传输对象/前端调用时传输
    - `VO` 表现对象，前端界面展示(后端 -> 前端)
    - `REQ/Request` 请求参数对象(前端 -> 后端)
- `贫血模型`、`充血模型`
...
- 开源协议/许可证 [1^] [^4]
    ...
    - [开源许可选择器](https://kaiyuanshe.cn/license-tool)，或者在创建gitee的时候可进行选择器操作
...
- [软件著作权登记](https://www.ccopyright.com.cn/)，参考：https://zhuanlan.zhihu.com/p/337166672
